...
adapter's own files. Canonical harness-core documentation lives in `../def/core/spec.md`;
each adapter's own specification lives in `../def/adapter/<host>/spec.md`.
...
deny/fail). Each adapter's `hooks.yaml` calls it with its own env id as the second argument —
...
2. Create `adapters/<new-env>/tools.yaml` — declare that host's tool names, write verbs, and
   payload keys (see `github-copilot/tools.yaml` for the shape).
